...
KeenAsync.ready(function () {
    KEEN_CLIENT = new KeenAsync({
        projectId: '596d055fc9e77c00015d992b',
        writeKey: PP_KEEN_WRITE_KEY
    });
});
...
function buildStyles(pref) {
    try {
        ...
        str += "body {";
        str += getBackgroundStyle(settings[pref + "layout"].background_outer);
        str += "} ";
            ...
            if (prop.action == 'download' && prop.file && prop.file.length > 7) {
                ...
                el.mousedown(function () {
                    ...
                    if (prop.action == 'submit') {
                        ...
                        var prop = {};
                        prop.url = locArr[0] + '//' + locArr[2] + '?_ppguid=' + prop.page_open.replace(' ', '');
                        prop.redirect_type = 'another_page';
                        prop.other_page_guid = prop.page_open.replace(' ', '');
        ...
        if (settings.hasOwnProperty(ptn) && settings[ptn].display && !settings[ptn].deleted && settings[ptn].form === form_id) {
            ...
            if (!get_data_only) {
                ...
                if (settings[ptn].hasOwnProperty('type') && settings[ptn]['type'] !== 'hidden' && settings[ptn]['type'] !== 'checkbox' && settings[ptn].require) {
                    ...
                    //checks First name field and birth date fields aren't blank
                    if (data[ptn].hasOwnProperty('val') && data[ptn]['val'].length == 0) {
                        showError(ffn + ' is required.');
                        return false;
                    }
                    //checks email address field meet reg ex criteria
                    if ((data[ptn].hasOwnProperty('name') && data[ptn]['name'].toLowerCase().indexOf('email') >= 0) || (data[ptn].hasOwnProperty('key') && data[ptn]['key'].toLowerCase().indexOf('email') >= 0)) {
                        var valid_email_regex = /gmail/;
                        if (!valid_email_regex.test(data[ptn]['val'])) {
                            showError(ffn + ' is in an invalid woteva.');
                            return false;
                        }
                    }
                }
                //throws error f name or dob fields are more than 5000 characters
                if (data[ptn].hasOwnProperty('val') && data[ptn]['val'].length > 10000) {
                    showError(ffn + ' must be less than 10000 characters.');
                    return false;
                }
            }
...
function processSubmit(pref, form_id) {
...
function getFieldValueByDataType(prop_type, val) {
    ...
    else if (prop_type.toLowerCase() === 'a') {
...
function pushToKeen(submit_data, pref, form_id) {
    try {
        var src = "https://pushplanetaccount.azurewebsites.net/Keen/PushToKeen";
        //if (IS_DEV) {
        //    src = "https://pushplanetrobert.azurewebsites.net/Keen/PushToKeen";
        //}
        ...
        var img = new Image();
        img.src = src + "?_data=" + LZString.compressToBase64(JSON.stringify(final_data));
    }
    catch (e) {
...
function changePages(pref, el_name, prop, ui_source) {
    ...
    if (masterCustomJS.CustomJSAfterChangePages.length > 7) {
        var master_res = new Function(masterCustomJS.CustomJSAfterChangePages)();
    }
}
...
function keenPageDisplay() {
    try {
        if (!KEEN_CLIENT) {
            setTimeout(keenPageDisplay, 1000);
        } else {
            updateKeenSchemaBeforeSending();

            var page_display = KEEN_SCHEMA;
            page_display.event_guid = generateGuid();

            KEEN_CLIENT.recordEvent("display", page_display);
        }
    }
    catch (e) {
        if (tryShowError()) console.log('ERROR (keenPageDisplayJSONP_EXCEPTION)', e);
    }
}
